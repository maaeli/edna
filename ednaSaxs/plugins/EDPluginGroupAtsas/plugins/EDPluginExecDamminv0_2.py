...
class EDPluginExecDamminv0_2(EDPluginExecProcessScript):
    ...
    def checkParameters(self):
        ...
        self.checkMandatoryParameters(self.dataInput.getGnomOutputFile(), "No GNOM output file specified")
    ...
    def checkDamminMode(self):
        try:
            if self.dataInput.getMode().value.upper() in ['FAST', 'SLOW']:
                self.__strMode = self.dataInput.getMode().value.upper()[0]
        except Exception:
            self.WARNING("Running DAMMIN in FAST mode by default")
    ...
    def checkDamminSymmetryInput(self):
        ...
        try:
            if self.dataInput.getSymmetry().value in knownSymmetry:
    ...
    def checkDamminDAMInput(self):
        DAMModel = ['S', 'E', 'C', 'P']
        try:
            if self.dataInput.getInitialDummyAtomModel().value in range(4):
                self.__strDAM = DAMModel[self.dataInput.getInitialDummyAtomModel().value]
                return
        except Exception:
            self.WARNING("No standard dummy atom model selected. Looking for a PDB mode file")
        else:
            try:
                tmpInputFileName = self.dataInput.getPdbInputFile().path.value
                os.symlink(tmpInputFileName, os.path.join(self.getWorkingDirectory(), "input.pdb"))
                self.__strDAM = 'input.pdb'
            except Exception:
                self.WARNING("Dummy atom model PDB file not specified. Using default model")


        #if self.dataInput.getInitialDummyAtomModel() is not None:
        #    if self.dataInput.getInitialDummyAtomModel().value not in range(5):
        #        self.ERROR("Unknown dummy atom model index specified")
        #        self.setFailure()
        #    else:
        #        if self.dataInput.getInitialDummyAtomModel().value == 4:
        #            if self.dataInput.getPdbInputFile() is None:
        #                self.ERROR("No PDB file for dummy atom model specified")
        #                self.setFailure()
    ...
    def checkParticleShapeInput(self):
        ...
        try:
            if self.dataInput.getExpectedParticleShape().value in range(3):
                self.__strParticleShape = particleShape[self.dataInput.getExpectedParticleShape().value]
        except Exception:
            self.ERROR("Using Unknown particle shape")
    ...
    def process(self, _edObject=None):
        ...
        self.DEBUG("EDPluginExecDamminv0_2.process")
...
#        self.DEBUG("EDPluginExecDamminv0_2.generateDamminScript")
#
#        tmpInputFileName = self.dataInput.getGnomOutputFile().path.value
...
#        self.setScriptCommandline(commandLine)
    ...
    def returnModelDimensions(self):
        ...
        if self.dataInput.getInitialDummyAtomModel().value in range(1, 4):
            return 3 * ['']
        if self.dataInput.getInitialDummyAtomModel().value is 4:
            return ['']
        else:
            return ''
